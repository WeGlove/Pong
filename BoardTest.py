...
class Board:
    ...
    def move_balls(self, speed):
        events = []
        remaining_balls_indexes = range(len(self.balls))
        speeds = [speed] * len(self.balls)
        intersections = [self.intersect(ball) for ball in self.balls]
        while not len(remaining_balls_indexes) == 0:
            min = None
            for i in range(len(remaining_balls_indexes)):
                if len(intersections[i]) == 0:
                    Ball_moved.Ball_moved(remaining_balls_indexes[i],)
                    del remaining_balls_indexes[i]
                    del speeds[i]
                    del intersections[i]
                    break
